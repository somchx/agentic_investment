...
def build_company_reasons(ticker: str, user_id: int | None = None, include_custom: bool = True):
    ...
    # reason_id is set to the real, globally-unique reason_key (not a
    # cosmetic "R1"/"R2"/"R3" display label) so a report's `key` field is
    # always directly usable by /api/screen and /api/positions without a
    # separate id-to-key mapping -- the old Flask UI had exactly this bug
    # (a frontend BUILTIN_KEY_MAP hack to translate "R1" -> "revenue_growth"
    # before it could be POSTed anywhere), fixed here at the source instead.
    reasons = [
        (
            Reason("revenue_growth", company_name, "Revenue", ">", 0.10, kind="yoy_growth",
                   description="Revenue growth must exceed 10% YoY"),
            revenue_facts, None,
        ),
        (
            Reason("operating_margin", company_name, "OperatingIncome", ">=", 0.20, kind="margin_level",
                   denominator_metric="Revenue",
                   description="Operating margin must stay at or above 20%"),
            oi_facts, revenue_facts,
        ),
        (
            Reason("debt_growth", company_name, "LongTermDebt", "<", 0.15, kind="yoy_growth",
    ...
    if include_custom and user_id is not None:
        import db
        for row in db.list_reasons(ticker=ticker, user_id=user_id):
            if row["is_builtin"]:
                continue
            custom_reason = Reason(
                row["reason_key"], company_name, row["metric"], row["comparison"], row["threshold"],
                kind=row["kind"], denominator_metric=row["denominator_metric"] or "",
                description=row["description"], instant=bool(row["instant"]), is_custom=True,
            )
            if custom_reason.instant:
                facts = extract_instant_facts_for_tag(facts_json, cik, custom_reason.metric)
                denom_facts = None
            else:
                facts = extract_quarterly_facts_for_tag(facts_json, cik, custom_reason.metric)
                denom_facts = (
                    extract_quarterly_facts_for_tag(facts_json, cik, custom_reason.denominator_metric)
                    if custom_reason.kind == "margin_level" else None
                )
            reasons.append((custom_reason, facts, denom_facts))
